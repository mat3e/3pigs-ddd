...
import static io.github.mat3e.fairytales.model.vo.Material.BRICKS
...
class HouseSpec extends Specification implements HouseHelpers {
    def 'should restore House from snapshot'() {
        ...
        def snapshot = new io.github.mat3e.fairytales.model.vo.HouseSnapshot(randomId(), material, pigs)
    ...
    def 'should NOT restore when more than 3 pigs'() {
        given:
        def snapshot = new io.github.mat3e.fairytales.model.vo.HouseSnapshot(randomId(), BRICKS, [LAZY, LAZY, LAZY, LAZY])
    ...
    def 'should throw when handling hurricane and from BRICKS'() {
        given:
        def house = House.from new io.github.mat3e.fairytales.model.vo.HouseSnapshot(randomId(), BRICKS, [NOT_LAZY])
    ...
    private static House houseWith(List<io.github.mat3e.fairytales.model.vo.Pig> pigs) {
        House.from new io.github.mat3e.fairytales.model.vo.HouseSnapshot(randomId(), BRICKS, pigs)
    }
}
