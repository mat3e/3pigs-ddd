...
import static io.github.mat3e.fairytales.model.vo.Pig.LAZY
...
class BigBadWolfServiceSpec extends Specification implements HouseHelpers {
    ...
    def 'should blow down house from'() {
        ...
        with(house.snapshot) {
            ...
            with(it.events()) {
                ...
                it[0] instanceof io.github.mat3e.fairytales.model.event.HouseAbandoned
                (it[0] as io.github.mat3e.fairytales.model.event.HouseAbandoned).refugees() == [LAZY]
            }
        }
    ...
    def 'should fail with house from BRICKS'() {
        ...
        then:
        1 * publisher.publish(_ as io.github.mat3e.fairytales.model.event.WolfResignedFromAttacking)
    }
